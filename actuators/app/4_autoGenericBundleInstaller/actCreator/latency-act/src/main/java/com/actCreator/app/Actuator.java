...
public class Actuator implements BundleActivator, Runnable {
  ...
  private enum COMMANDS {
    OPTIMIZE, // For latency, optimize means decreasing the value
    DEGRADE // For latency, degrade means increasing the value
  }

  // MQTT
  final String CLIENT_ID = "act_latency";
  final String GENERAL_TOPIC = "Network/latency";
  final String COMMANDS_TOPIC = GENERAL_TOPIC + "/commands";

  // JSON env file
  final String ENV_FILE_PATH = "/simulated_env/env.json";
  final String ENV_NODE = "latency";
  ...
  @Override
  public void run() {
    System.out.println(debugInfo + "I'm running, I'm the latency actuator");
    System.out.println("------------------------------------------------------------");
      ...
      client.setCallback(new MqttCallback() {
        ...
        public void messageArrived(String topic, MqttMessage message) {
          ...
          if (msgText.equals(COMMANDS.OPTIMIZE.name())) {
            optimizeCommand(); // Latency improvement means decreasing the value
          } else if (msgText.equals(COMMANDS.DEGRADE.name())) {
            degradeCommand(); // Latency worsening means increasing the value
          } else {
  ...
  /**
   * Attempts to optimize the latency value.
   * For latency, optimization means reducing the value.
   *
   * @return 0 if successful, 1 if MIN_VAL exceeded, 2 if JSON operation failed.
   */
  private int optimizeCommand() {
    System.out.println(debugInfo + "OPTIMIZING latency (decreasing value)...");
    ...
    try {
      ...
      if (map.containsKey(ENV_NODE)) {
        ...
        if (valueObj instanceof Number) {
          ...
          // For latency, optimize means decrease
          if ((currentValue - 10) < MIN_VAL) {
            System.err.println(ANSI_RED + "OPTIMIZE: MIN_VAL Exceeded. Cannot decrease latency further." + ANSI_RESET);
            return 1;
          }
      ...
      System.out.println(ANSI_GREEN + "Latency value optimized (decreased)." + ANSI_RESET);
  ...
  /**
   * Attempts to degrade the latency value.
   * For latency, degradation means increasing the value.
   *
   * @return 0 if successful, 1 if MAX_VAL exceeded, 2 if JSON operation failed.
   */
  private int degradeCommand() {
    System.out.println(debugInfo + "DEGRADING latency (increasing value)...");
    ...
    try {
      ...
      if (map.containsKey(ENV_NODE)) {
        ...
        if (valueObj instanceof Number) {
          double currentValue = ((Number) valueObj).doubleValue();
          // For latency, degrade means increase
          if ((currentValue + 10) > MAX_VAL) {
            System.err.println(ANSI_RED + "DEGRADE: MAX_VAL Exceeded. Cannot increase latency further." + ANSI_RESET);
            return 1;
          }
          newValue = currentValue + 10.0;
          innerMap.put(NODE_VALUE, newValue);
      ...
      mapper.writerWithDefaultPrettyPrinter().writeValue(file, map);
      System.out.println(ANSI_GREEN + "Latency value degraded (increased)." + ANSI_RESET);
